/**
 * presskey-cursorfree-steer — under UMBRIEL_CURSOR=never, press_key on a control with NO native window handle (a WinUI/
 ...
 *
 * bun test is broken repo-wide — runnable harness (MCP subprocess started with UMBRIEL_CURSOR=never; taskbar, nothing to close):
...
try {
  ...
  assert(/no key path|by intent|invoke \/ select \/ set_value/i.test(focusDesc) && /UMBRIEL_CURSOR=never/.test(focusDesc), `focus description carries the cursor=never no-key-path caveat (got: ${JSON.stringify(focusDesc.slice(-140))})`);
  ...
  if (buttonRef === undefined) console.log('  skip: no no-handle taskbar Button ref to exercise the steer');
    ...
    assert(pressed.result?.isError === true && /UMBRIEL_CURSOR=never/.test(text), 'press_key on a no-handle control is refused under never');
