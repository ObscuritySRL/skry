/**
 * cursor-lockdown — UMBRIEL_CURSOR=never must be ENFORCED on every SendInput path: the MOUSE tools (click_point,
 ...
 *
 * Proof (drives the real MCP server with UMBRIEL_CURSOR=never): every real-cursor path refuses with NO input
...
const proc = Bun.spawn(['bun', 'run', `${import.meta.dir}/../mcp.ts`], { stdin: 'pipe', stdout: 'pipe', stderr: 'ignore', env: { ...Bun.env, UMBRIEL_PROFILE: 'full', UMBRIEL_CURSOR: 'never' } });
...
try {
  ...
  assert(isErr(explicit) && /UMBRIEL_CURSOR=never/.test(textOf(explicit)), 'click_point {cursor:true} is REFUSED under UMBRIEL_CURSOR=never (no real cursor moved)');
  ...
  assert(isErr(fallback) && /UMBRIEL_CURSOR=never/.test(textOf(fallback)), 'click_point real-cursor FALLBACK (posted click reached no window) is refused under UMBRIEL_CURSOR=never');
  ...
  assert(isErr(held) && /UMBRIEL_CURSOR=never/.test(textOf(held)), 'hold_key (SendInput key-down) is refused under UMBRIEL_CURSOR=never (no key held)');

  const chord = await call('tools/call', { name: 'press_key', arguments: { key: 'Control+a' } });
  assert(isErr(chord) && /UMBRIEL_CURSOR=never/.test(textOf(chord)), 'press_key chord (SendInput to the focused control) is refused under UMBRIEL_CURSOR=never (no keys sent)');
  ...
  if (buttonRef === undefined) console.log('  skip: no no-handle taskbar Button ref to exercise type/paste refusal');
    ...
    assert(isErr(typed) && /UMBRIEL_CURSOR=never/.test(textOf(typed)) && /set_value/.test(textOf(typed)), 'type on a no-handle control (SendInput) is refused under UMBRIEL_CURSOR=never and steers to set_value');

    const pastedRef = await call('tools/call', { name: 'paste', arguments: { ref: buttonRef, text: 'leak' } });
    assert(isErr(pastedRef) && /UMBRIEL_CURSOR=never/.test(textOf(pastedRef)), 'paste on a no-handle control (SendInput Ctrl+V) is refused under UMBRIEL_CURSOR=never');
  }
  ...
  assert(isErr(pasted) && /UMBRIEL_CURSOR=never/.test(textOf(pasted)), 'paste with no ref (Ctrl+V via SendInput) is refused under UMBRIEL_CURSOR=never (nothing pasted)');

  const copied = await call('tools/call', { name: 'copy', arguments: {} });
  assert(isErr(copied) && /UMBRIEL_CURSOR=never/.test(textOf(copied)), 'bare copy (Ctrl+C via SendInput) is refused under UMBRIEL_CURSOR=never');
  ...
  assert(isErr(acted) && /UMBRIEL_CURSOR=never/.test(textOf(acted)), 'find_and_act {do:type} routes through act() and is refused under UMBRIEL_CURSOR=never (the act-type bypass is closed)');
} finally {
  proc.kill();
}
...
console.log(
  failures === 0
    ? '\nPASS — UMBRIEL_CURSOR=never is enforced on every SendInput path: mouse (click_point ×2, click_text/drag share the guard), keyboard (hold_key, press_key chord, type→set_value), clipboard (paste, bare copy), and find_and_act/reveal {do:type}.'
