...
test('NO handler echoes a raw control name — every element/match/description/mark name reaching the model is redacted', () => {
  ...
  for (const raw of ['JSON.stringify(element.name)', 'JSON.stringify(element.cachedName)', 'JSON.stringify(match.name)', 'JSON.stringify(description.name)', 'JSON.stringify(mark.name)', 'JSON.stringify(root.name)', 'JSON.stringify(toast.label)', 'JSON.stringify(trayFlyout.label)']) {
    expect(mcp).not.toContain(raw);
  }
  // the desktop_snapshot {root} scope HEADER and the list_windows toast/tray-flyout labels are built OUTSIDE the
  // renderTree chokepoint (a toast label is a UIA name = announced content, not a window title), so each masks itself.
  expect(mcp).toContain('scoped to ${JSON.stringify(redactSecrets(root.name))}');
  expect(mcp).toContain('JSON.stringify(redactSecrets(toast.label))');
  expect(mcp).toContain('JSON.stringify(redactSecrets(trayFlyout.label))');
});
